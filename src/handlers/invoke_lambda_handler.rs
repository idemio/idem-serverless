...
impl Config for AWSLambdaFunctionProxyHandlerConfig {
...
impl AWSLambdaFunctionProxyHandler {
    pub(crate) async fn new(config: AWSLambdaFunctionProxyHandlerConfig) -> Self {
        Self {
                        ...
                        .await
                    {
                        Ok(response) => {
                            ...
                            let lambda_response: ApiGatewayProxyResponse =
                                serde_json::from_slice(&response_payload_bytes).unwrap_or_else(|_| {
                                    todo!("failed to get response from lambda function call.")
                                });
